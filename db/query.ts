...
import { RECEIVE_FT } from './queries/ft';
...
export default async function query_all(startDate: string, endDate: string, accountIds: Set<string>) {
    ...
    const [rcv_sent_near, ft_receive] = await Promise.all([rcv_sent_near_promise, ft_receive_promise]);
    ...
    for (const row of rcv_sent_near.rows) {
        const r = <Row>{
            ...
            receiver_owner_account: row.receiver_owner_account,
