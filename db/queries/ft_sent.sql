SELECT
    ...
    'Fungible tokens - Send token' transaction_type,
...
WHERE
    ...
    AND (
        SELECT
            count(*)
        FROM
            jsonb_object_keys(COALESCE(ra.args -> 'args_json'::text, '{}')::jsonb)) IN (2, 3)
...
SELECT
    ...
    WHERE
        ...
        AND (
            SELECT
                count(*)
            FROM jsonb_object_keys(COALESCE(ra.args -> 'args_json'::text, '{}')::jsonb)) IN (3, 4)
        AND (ra.args -> 'args_json'::text) ->> 'amount'::text ~ '^[0-9]+$'
